...
It is listening for the [issueEvent](/integrations/apis/github/events/issues) GitHub webhook, and when it receives one, we will take care of calling the `run` function supplied to the `Trigger` constructor with the webhook payload. This gives you the following advantages over traditional webhooks:
