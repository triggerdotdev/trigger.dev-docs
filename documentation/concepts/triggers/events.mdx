...
There are three ways of sending an event that will Trigger a Job.

### 1. From your own code

You can use `client.sendEvent()` to send an event from your own code. [View the SDK reference](/sdk/client).
...
You can use `io.sendEvent()` to send events from inside a Job, to trigger another. [View the SDK reference](/sdk/io).
