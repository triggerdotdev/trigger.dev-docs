...
- From the backend you can get the latest runs for a Job using the [client.getRuns()](/sdk/getruns) method. This will return an array of runs, including their ids. You can then pass an id to your frontend to display the live status.

<Note>
  You can get the full status of a Run from the backend using the
  [client.getRun()](/sdk/getrun) method.
</Note>
